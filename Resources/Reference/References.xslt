...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:r="http://git.erwinefamily.net/FsInfoCat/V1/References.xsd"
    ...
    <xsl:template match="/">
        <xsl:element name="References">
            <xsl:text>APA References
    /// </xsl:text><xsl:element name="list">
    ...
    <xsl:template match="b:Source" mode="Reference">
        <xsl:variable name="primaryContributors"><xsl:apply-templates select="." mode="GetPrimaryContributors" /></xsl:variable>
        <xsl:variable name="title">
            <xsl:variable name="title" select="normalize-space(b:Title)"/>
            <xsl:choose>
                <xsl:when test="string-length($title)=0"><xsl:apply-templates select="." mode="GetEffectiveShortTitle" /></xsl:when>
                <xsl:otherwise><xsl:value-of select="$title"/></xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <xsl:variable name="shortTitle">
            <xsl:variable name="shortTitle"><xsl:apply-templates select="." mode="GetEffectiveShortTitle" /></xsl:variable>
            <xsl:if test="not($shortTitle=$title)"><xsl:value-of select="$shortTitle"/></xsl:if>
        </xsl:variable>
        <xsl:variable name="effectiveOrg"><xsl:apply-templates select="." mode="GetEffectiveOrg" /></xsl:variable>
        ...
        <xsl:variable name="url"><xsl:variable name="url" select="normalize-space(b:URL)" /><xsl:if test="not($url=$title)"><xsl:value-of select="$url"/></xsl:if></xsl:variable>
        <xsl:variable name="doi"><xsl:variable name="doi" select="normalize-space(b:DOI)" /><xsl:if test="not($doi=$title)"><xsl:value-of select="concat('doi: ', $doi)"/></xsl:if></xsl:variable>
        ...
        <xsl:variable name="orgElements">
            <xsl:choose>
                <xsl:when test="./b:SourceType='Book' or ./b:SourceType='BookSection' or ./b:SourceType='Report'">
                    <xsl:element name="b:Institution"><xsl:value-of select="normalize-space(b:Publisher)"/></xsl:element>
                </xsl:when>
                <xsl:when test="./b:SourceType='JournalArticle' or ./b:SourceType='ArticleInAPeriodical'">
                    <xsl:element name="b:Institution"><xsl:value-of select="normalize-space(b:JournalName)"/></xsl:element>
                    <xsl:element name="b:Institution"><xsl:value-of select="normalize-space(b:Publisher)"/></xsl:element>
                </xsl:when>
                <xsl:when test="./b:SourceType='ConferenceProceedings'">
                    <xsl:element name="b:Institution"><xsl:value-of select="normalize-space(b:ConferenceName)"/></xsl:element>
                </xsl:when>
                <xsl:when test="./b:SourceType='Performance'">
                    <xsl:element name="b:Institution"><xsl:value-of select="normalize-space(b:Theater)"/></xsl:element>
                </xsl:when>
                <xsl:when test="./b:SourceType='Art'">
                    <xsl:element name="b:Institution"><xsl:value-of select="normalize-space(b:Institution)"/></xsl:element>
                </xsl:when>
            </xsl:choose>
            <xsl:element name="b:Institution"><xsl:value-of select="normalize-space(b:ProductionCompany)"/></xsl:element>
            <xsl:element name="b:Institution"><xsl:value-of select="normalize-space(b:Institution)"/></xsl:element>
        </xsl:variable>
    ...
    <xsl:template match="b:Source" mode="GetEffectiveShortTitle">
        <xsl:variable name="titleElements">
            <xsl:element name="b:ShortTitle"><xsl:value-of select="normalize-space(b:ShortTitle)"/></xsl:element>
            <xsl:choose>
                <xsl:when test="./b:SourceType='Book' or ./b:SourceType='BookSection'">
                    <xsl:element name="b:ShortTitle"><xsl:value-of select="normalize-space(b:BookTitle)"/></xsl:element>
                </xsl:when>
                <xsl:when test="./b:SourceType='ArticleInAPeriodical'">
                    <xsl:element name="b:ShortTitle"><xsl:value-of select="normalize-space(b:PeriodicalTitle)"/></xsl:element>
                </xsl:when>
                <xsl:when test="./b:SourceType='InternetSite' or ./b:SourceType='DocumentFromInternetSite'">
                    <xsl:element name="b:ShortTitle"><xsl:value-of select="normalize-space(b:InternetSiteTitle)"/></xsl:element>
                </xsl:when>
                <xsl:when test="./b:SourceType='ElectronicSource' or ./b:SourceType='Art' or ./b:SourceType='Misc'">
                    <xsl:element name="b:ShortTitle"><xsl:value-of select="normalize-space(b:PublicationTitle)"/></xsl:element>
                </xsl:when>
                <xsl:when test="./b:SourceType='SoundRecording'">
                    <xsl:element name="b:ShortTitle"><xsl:value-of select="normalize-space(b:AlbumTitle)"/></xsl:element>
                </xsl:when>
                <xsl:when test="./b:SourceType='Interview'">
                    <xsl:element name="b:ShortTitle"><xsl:value-of select="normalize-space(b:BroadcastTitle)"/></xsl:element>
                </xsl:when>
            </xsl:choose>
        </xsl:variable>
    ...
    <xsl:template match="b:Person" mode="ToNameElement">
        ...
        <xsl:choose>
            <xsl:when test="not(string-length($middle)=0)">
                <xsl:element name="Name"><xsl:value-of select="concat($last, ', ', $first, ' ', $middle)"/></xsl:element>
            </xsl:when>
            <xsl:when test="not(string-length($first)=0)">
                <xsl:element name="Name"><xsl:value-of select="concat($last, ', ', $first)"/></xsl:element>
            </xsl:when>
            <xsl:when test="not(string-length($last)=0)">
                <xsl:element name="Name"><xsl:value-of select="$last"/></xsl:element>
            </xsl:when>
        </xsl:choose>
    </xsl:template>
    ...
    <xsl:template name="GetNormalizedYear">
        ...
        <xsl:if test="string-length($normalized)&gt;0 and contains($Digits, substring($normalized, 1, 1))">
            ...
            <xsl:choose>
                <xsl:when test="$y&gt;999 and $y &lt;10000"><xsl:value-of select="string($y)"/></xsl:when>
                <xsl:when test="string-length(string($y))=2">
                    <xsl:choose>
                        <xsl:when test="$y&lt;40"><xsl:value-of select="concat('20', string($y))"/></xsl:when>
                        <xsl:otherwise><xsl:value-of select="concat('19', string($y))"/></xsl:otherwise>
                    </xsl:choose>
                </xsl:when>
            </xsl:choose>
        </xsl:if>
    ...
    <xsl:template name="GetNormalizedMonth">
        ...
        <xsl:choose>
            <xsl:when test="$lc='january' or $lc='jan'"><xsl:text>January</xsl:text></xsl:when>
            <xsl:when test="$lc='february' or $lc='feb'"><xsl:text>February</xsl:text></xsl:when>
            <xsl:when test="$lc='march' or $lc='mar'"><xsl:text>March</xsl:text></xsl:when>
            <xsl:when test="$lc='april' or $lc='apr'"><xsl:text>April</xsl:text></xsl:when>
            <xsl:when test="$lc='may'"><xsl:text>May</xsl:text></xsl:when>
            <xsl:when test="$lc='june' or $lc='jun'"><xsl:text>June</xsl:text></xsl:when>
            <xsl:when test="$lc='july' or $lc='jul'"><xsl:text>July</xsl:text></xsl:when>
            <xsl:when test="$lc='august' or $lc='aug'"><xsl:text>August</xsl:text></xsl:when>
            <xsl:when test="$lc='september' or $lc='sep' or $lc='sept'"><xsl:text>September</xsl:text></xsl:when>
            <xsl:when test="$lc='october' or $lc='oct'"><xsl:text>October</xsl:text></xsl:when>
            <xsl:when test="$lc='november' or $lc='nov'"><xsl:text>November</xsl:text></xsl:when>
            <xsl:when test="$lc='december' or $lc='dec'"><xsl:text>December</xsl:text></xsl:when>
            <xsl:when test="string-length($lc)&gt;0 and contains($Digits, substring($lc, 1, 1))">
                <xsl:variable name="m" select="number($lc)" />
                <xsl:choose>
                    <xsl:when test="$m=1"><xsl:text>January</xsl:text></xsl:when>
                    <xsl:when test="$m=2"><xsl:text>February</xsl:text></xsl:when>
                    <xsl:when test="$m=3"><xsl:text>March</xsl:text></xsl:when>
                    <xsl:when test="$m=4"><xsl:text>April</xsl:text></xsl:when>
                    <xsl:when test="$m=5"><xsl:text>May</xsl:text></xsl:when>
                    <xsl:when test="$m=6"><xsl:text>June</xsl:text></xsl:when>
                    <xsl:when test="$m=7"><xsl:text>July</xsl:text></xsl:when>
                    <xsl:when test="$m=8"><xsl:text>August</xsl:text></xsl:when>
                    <xsl:when test="$m=9"><xsl:text>September</xsl:text></xsl:when>
                    <xsl:when test="$m=10"><xsl:text>October</xsl:text></xsl:when>
                    <xsl:when test="$m=11"><xsl:text>November</xsl:text></xsl:when>
                    <xsl:when test="$m=12"><xsl:text>December</xsl:text></xsl:when>
                </xsl:choose>
            </xsl:when>
        </xsl:choose>
    </xsl:template>
    <xsl:template name="GetMonthNumber">
        ...
        <xsl:choose>
            <xsl:when test="$lc='january' or $lc='jan'"><xsl:value-of select="1"/></xsl:when>
            <xsl:when test="$lc='february' or $lc='feb'"><xsl:value-of select="2"/></xsl:when>
            <xsl:when test="$lc='march' or $lc='mar'"><xsl:value-of select="3"/></xsl:when>
            <xsl:when test="$lc='april' or $lc='apr'"><xsl:value-of select="4"/></xsl:when>
            <xsl:when test="$lc='may'"><xsl:value-of select="5"/></xsl:when>
            <xsl:when test="$lc='june' or $lc='jun'"><xsl:value-of select="6"/></xsl:when>
            <xsl:when test="$lc='july' or $lc='jul'"><xsl:value-of select="7"/></xsl:when>
            <xsl:when test="$lc='august' or $lc='aug'"><xsl:value-of select="8"/></xsl:when>
            <xsl:when test="$lc='september' or $lc='sep' or $lc='sept'"><xsl:value-of select="9"/></xsl:when>
            <xsl:when test="$lc='october' or $lc='oct'"><xsl:value-of select="10"/></xsl:when>
            <xsl:when test="$lc='november' or $lc='nov'"><xsl:value-of select="11"/></xsl:when>
            <xsl:when test="$lc='december' or $lc='dec'"><xsl:value-of select="12"/></xsl:when>
            <xsl:when test="string-length($lc)&gt;0 and contains($Digits, substring($lc, 1, 1))">
                <xsl:variable name="m" select="number($lc)" />
                <xsl:if test="$m&gt;0 and $m&lt;13"><xsl:value-of select="$m"/></xsl:if>
            </xsl:when>
        </xsl:choose>
    </xsl:template>
    <xsl:template name="GetNormalizedDay">
        ...
        <xsl:if test="string-length($normalized)&gt;0 and contains($Digits, substring($normalized, 1, 1))">
            ...
            <xsl:choose>
                <xsl:when test="$d&gt;0 and $d&lt;10"><xsl:value-of select="concat('0', string($d))"/></xsl:when>
                <xsl:when test="$d&gt;9 and $d&lt;30"><xsl:value-of select="string($d)"/></xsl:when>
                <xsl:when test="string-length($m)=0">
                    <xsl:if test="$d=30 or $d=31"><xsl:value-of select="string($d)"/></xsl:if>
                </xsl:when>
                <xsl:when test="$m=4 or $m=6 or $m=9 or $m=11">
                    <xsl:if test="$d=30"><xsl:value-of select="string($d)"/></xsl:if>
                </xsl:when>
                <xsl:when test="not($m=2)"><xsl:value-of select="string($d)"/></xsl:when>
            </xsl:choose>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template name="RenderItemElement">
        ...
        <xsl:choose>
            ...
            <xsl:text>
    ///     </xsl:text><xsl:element name="item"><xsl:element name="term"><xsl:value-of select="$line1Left"/></xsl:element><xsl:text>
    ///         </xsl:text><xsl:element name="description"><xsl:value-of select="$line1Right"/></xsl:element><xsl:text>
    ///     </xsl:text></xsl:element>
            </xsl:when>
            <xsl:otherwise>
                <xsl:text>
    ///     </xsl:text><xsl:element name="item"><xsl:element name="term"><xsl:value-of select="$line1Left"/></xsl:element><xsl:text>
    ///         </xsl:text><xsl:element name="description"><xsl:value-of select="$line1Right"/><xsl:text>
    ///         </xsl:text><xsl:element name="para"><xsl:value-of select="$line2"/></xsl:element><xsl:if test="not(string-length(normalize-space($line3))=0)"><xsl:text>
    ///         </xsl:text><xsl:element name="para"><xsl:value-of select="$line3"/></xsl:element></xsl:if></xsl:element><xsl:text>
    ///     </xsl:text></xsl:element>
            </xsl:otherwise>
        </xsl:choose>
    </xsl:template>
