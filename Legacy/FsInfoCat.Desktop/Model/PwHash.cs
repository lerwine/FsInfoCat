...
namespace FsInfoCat.Desktop.Model
{
    ...
    public struct PwHash : IEquatable<PwHash>, IEquatable<string>
    {
        ...
        private static byte[] ComputeHash(SecureString securePassword, byte[] salt)
        {
            using (SHA512 sha = SHA512.Create())
            {
                ...
                try
                {
                    sha.ComputeHash(Encoding.ASCII.GetBytes(Marshal.PtrToStringBSTR(ptr)).Concat(salt).ToArray());
                    return sha.Hash;
                }
        ...
        public static bool TryCreate(string base64EncodedHash, out PwHash? result)
        {
            ...
            {
                _logger.LogError(exc, "Error decoding \"{base64EncodedHash}\": {Message}", base64EncodedHash,
                    exc.Message);
                result = default;
                return false;
            }
            if (data.Length == TOTAL_BYTES_LENGTH)
            {
                try
                {
                    result = new PwHash(data);
                    return true;
                }
                catch (Exception exception)
                {
                    _logger.LogError(exception, "Error instantiating value from \"{base64EncodedHash}\": {Message}", base64EncodedHash, exception.Message);
                }
            }
            else
                _logger.LogError("Invalid data length decoded from \"{base64EncodedHash}\"", base64EncodedHash);
            result = null;
            return false;
        }

        public static bool TryCreate(SecureString securePassword, out PwHash? result)
        {
            if (securePassword is null || securePassword.Length == 0)
            {
                result = null;
                return true;
            }
            try
            {
                ...
                result = new PwHash(ComputeHash(securePassword, salt), salt);
                return true;
            }
            catch (Exception exception)
            {
                _logger.LogError(exception, "Error creating value from \"{securePassword}\": {Message}", securePassword, exception.Message);
        ...
        public static bool TryCreate(SecureString securePassword, ulong saltBits, out PwHash? result)
        {
            if (securePassword is null || securePassword.Length == 0)
            {
                result = null;
            ...
            try
            {
                byte[] salt = BitConverter.GetBytes(saltBits);
                using (RNGCryptoServiceProvider cryptoServiceProvider = new RNGCryptoServiceProvider())
                    cryptoServiceProvider.GetBytes(salt);
                result = new PwHash(ComputeHash(securePassword, salt), salt);
                return true;
        ...
        public static bool Test(PwHash? hash, SecureString securePassword)
        {
            return (hash.HasValue) ? hash.Value.Test(securePassword) : (securePassword is null || securePassword.Length == 0);
        }

        public static bool Test(string base64EncodedHash, SecureString securePassword)
        {
            if (string.IsNullOrWhiteSpace(base64EncodedHash))
                return securePassword is null || securePassword.Length == 0;
            byte[] data;
            try { data = Convert.FromBase64String(base64EncodedHash); }
            ...
            byte[] hash = ComputeHash(securePassword, salt);
        ...
        public bool Test(SecureString securePassword)
        {
            if (securePassword is null || securePassword.Length == 0)
                return false;
            byte[] hash = ComputeHash(securePassword, GetSaltBytes());
        ...
        public override string ToString() => Convert.ToBase64String(GetHashAndSaltBytes().ToArray());
