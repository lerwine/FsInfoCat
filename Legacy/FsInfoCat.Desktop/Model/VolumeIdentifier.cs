...
namespace FsInfoCat.Desktop.Model
{
    ...
    public struct VolumeIdentifier : IEquatable<VolumeIdentifier>
    {
        ...
        public VolumeIdentifier(string url) : this()
        {
            ...
            {
                string[] segments = Uri.UnescapeDataString(uri.AbsolutePath).Split(new char[] { UriHelper.URI_SCHEME_SEPARATOR_CHAR }, 3);
            ...
            else if (uri.Scheme.Equals(Uri.UriSchemeFile))
            {
                ...
                if (uri.IsDefaultPort && uri.Host.Length > 0)
                {
                    ...
                    if (path != uri.AbsolutePath)
                        uri.TrySetPathComponent(path.EndsWith(UriHelper.URI_PATH_SEPARATOR_STRING) ? path : $"{path}/", out uri);
                    else if (!path.EndsWith(UriHelper.URI_PATH_SEPARATOR_STRING))
        ...
        public static bool TryCreate(object obj, out VolumeIdentifier volumeIdentifer)
        {
            ...
            else if (obj is float f)
            {
                ...
                volumeIdentifer = new VolumeIdentifier(b);
            ...
            else if (obj is short ss)
            {
                ...
                volumeIdentifer = new VolumeIdentifier(us);
            else if (obj is Win32_LogicalDisk logicalDisk)
            {
                if (logicalDisk.DriveType == System.IO.DriveType.Network)
                {
                    if (!(Uri.TryCreate(logicalDisk.ProviderName, UriKind.Absolute, out Uri p) && (volumeIdentifer = new VolumeIdentifier(p.AbsoluteUri)).IsValid()))
                    {
                        volumeIdentifer = Empty;
                        return false;
                    }
                }
                else if (string.IsNullOrWhiteSpace(logicalDisk.VolumeSerialNumber) || !(volumeIdentifer = new VolumeIdentifier(logicalDisk.VolumeSerialNumber)).IsValid())
                {
                    volumeIdentifer = Empty;
                    return false;
                }
