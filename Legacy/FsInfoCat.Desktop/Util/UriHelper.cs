    ...
    public static class UriHelper
    {
        ...
        public static string GetUserNameAndPassword(this Uri uri, out string password)
        {
            if (!(uri is null) && uri.IsAbsoluteUri)
            {
                ...
                {
                    string[] upw = uri.GetComponents(UriComponents.UserInfo, UriFormat.UriEscaped).Split(new char[] { URI_SCHEME_SEPARATOR_CHAR }, 2);
        ...
        public static bool TrySetTrailingEmptyPathSegment(this Uri uri, bool shouldHaveTrailingSlash, out Uri result)
        {
            ...
            if (string.IsNullOrEmpty(path))
            {
                if (shouldHaveTrailingSlash)
                    return uri.TrySetPathComponent(URI_PATH_SEPARATOR_STRING, out result) && result.GetPathComponent().EndsWith(URI_PATH_SEPARATOR_STRING);
            }
            else if (path.EndsWith(URI_PATH_SEPARATOR_STRING) != shouldHaveTrailingSlash)
            {
                if (shouldHaveTrailingSlash)
                    return uri.TrySetPathComponent($"{path}/", out result) && result.GetPathComponent().EndsWith(URI_PATH_SEPARATOR_STRING);
                int i = path.Length - 1;
                while (i > 0 && path[i - 1] == URI_PATH_SEPARATOR_CHAR)
                    i--;
                return uri.TrySetPathComponent(path.Substring(0, i), out result) && !result.GetPathComponent().EndsWith(URI_PATH_SEPARATOR_STRING);
            }
