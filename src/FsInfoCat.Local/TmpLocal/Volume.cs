...
#pragma warning restore CS0659 // Type overrides Object.Equals(object o) but does not override Object.GetHashCode()
    {
        ...
        M.IFileSystem M.IVolume.FileSystem { get => FileSystem; }

        ILocalSubdirectory ILocalVolume.RootDirectory => RootDirectory;

        M.ISubdirectory M.IVolume.RootDirectory => RootDirectory;

        IEnumerable<ILocalVolumeAccessError> ILocalVolume.AccessErrors => AccessErrors.Cast<ILocalVolumeAccessError>();

        IEnumerable<M.IVolumeAccessError> M.IVolume.AccessErrors => AccessErrors.Cast<M.IVolumeAccessError>();

        IEnumerable<ILocalPersonalVolumeTag> ILocalVolume.PersonalTags => PersonalTags.Cast<ILocalPersonalVolumeTag>();

        IEnumerable<M.IPersonalVolumeTag> M.IVolume.PersonalTags => PersonalTags.Cast<M.IPersonalVolumeTag>();

        IEnumerable<ILocalSharedVolumeTag> ILocalVolume.SharedTags => SharedTags.Cast<ILocalSharedVolumeTag>();

        IEnumerable<M.ISharedVolumeTag> M.IVolume.SharedTags => SharedTags.Cast<M.ISharedVolumeTag>();
        ...
        internal static void OnBuildEntity(EntityTypeBuilder<Volume> builder)
        {
            ...
            _ = builder.Property(nameof(Identifier)).HasConversion(M.VolumeIdentifier.Converter);
        }
...
#pragma warning restore CS1591 // Missing XML comment for publicly visible type or member
        {
            ...
            if (diskInfo is null)
            {
                ...
                result.Status = M.VolumeStatus.Unknown;
        ...
        /// Tests whether the current database entity is equal to another.
        /// </summary>
        /// <param name="other">The <see cref="M.IVolume" /> to compare to.</param>
        /// <returns><see langword="true" /> if the <paramref name="other"/> entity is equal to the current entity; otherwise, <see langword="false" />.</returns>
        public bool Equals(M.IVolume other)
        {
            if (other is null) return false;
            if (other is Volume volume) return Equals(volume);
            if (TryGetId(out Guid id1)) return other.TryGetId(out Guid id2) && id1.Equals(id2);
...
#pragma warning restore CS1591 // Missing XML comment for publicly visible type or member
        {
            ...
            return obj is M.IVolumeRow row && (TryGetId(out Guid id1) ? row.TryGetId(out Guid id2) && id1.Equals(id2) :
        ...
        /// </summary>
        /// <param name="fileSystemId">The <see cref="IHasSimpleIdentifier.Id"/> of the associated <see cref="M.IFileSystem"/>.</param>
