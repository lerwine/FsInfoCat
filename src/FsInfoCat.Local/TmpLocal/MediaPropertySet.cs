...
#pragma warning disable CS1591 // Missing XML comment for publicly visible type or member
            ...
            CancellationToken cancellationToken)
        {
            ...
            M.IMediaProperties currentProperties = await fileDetailProvider.GetMediaPropertiesAsync(cancellationToken);
        ...
        public bool Equals(M.IMediaPropertySet other)
        {
            if (other is null) return false;
            if (other is MediaPropertySet propertySet) return Equals(propertySet);
            if (TryGetId(out Guid id)) return other.TryGetId(out Guid id2) && id.Equals(id2);
        ...
        public override bool Equals(M.IMediaPropertiesRow other)
        {
            if (other is null) return false;
            if (other is MediaPropertySet propertySet) return Equals(propertySet);
            if (TryGetId(out Guid id)) return other.TryGetId(out Guid id2) && id.Equals(id2);
        ...
        public override bool Equals(M.IMediaProperties other)
        {
            if (other is null) return false;
            if (other is MediaPropertySet propertySet) return Equals(propertySet);
            if (other is M.IMediaPropertiesRow row)
            {
        ...
        public override bool Equals(object obj)
        {
            ...
            if (obj is M.IMediaPropertiesRow row)
            {
                if (TryGetId(out Guid id)) return row.TryGetId(out Guid id2) && id.Equals(id2);
                return !row.TryGetId(out _) && (row is ILocalMediaPropertiesRow localRow) ? ArePropertiesEqual(localRow) : ArePropertiesEqual(row);
            }
            return obj is M.IMediaProperties properties && ArePropertiesEqual(properties);
        }
    }
