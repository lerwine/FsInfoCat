...
#pragma warning disable CS1591 // Missing XML comment for publicly visible type or member
            ...
            CancellationToken cancellationToken)
        {
            ...
            M.IDRMProperties currentProperties = await fileDetailProvider.GetDRMPropertiesAsync(cancellationToken);
        ...
        public bool Equals(M.IDRMPropertySet other)
        {
            if (other is null) return false;
            if (other is DRMPropertySet propertySet) return Equals(propertySet);
            if (TryGetId(out Guid id)) return other.TryGetId(out Guid id2) && id.Equals(id2);
        ...
        public override bool Equals(M.IDRMPropertiesRow other)
        {
            if (other is null) return false;
            if (other is DRMPropertySet propertySet) return Equals(propertySet);
            if (TryGetId(out Guid id)) return other.TryGetId(out Guid id2) && id.Equals(id2);
        ...
        public override bool Equals(M.IDRMProperties other)
        {
            if (other is null) return false;
            if (other is DRMPropertySet propertySet) return Equals(propertySet);
            if (other is M.IDRMPropertiesRow row)
            {
        ...
        public override bool Equals(object obj)
        {
            ...
            if (obj is M.IDRMPropertiesRow row)
            {
                if (TryGetId(out Guid id)) return row.TryGetId(out Guid id2) && id.Equals(id2);
                return !row.TryGetId(out _) && (row is ILocalDRMPropertiesRow localRow) ? ArePropertiesEqual(localRow) : ArePropertiesEqual(row);
            }
            return obj is M.IDRMProperties properties && ArePropertiesEqual(properties);
        }
