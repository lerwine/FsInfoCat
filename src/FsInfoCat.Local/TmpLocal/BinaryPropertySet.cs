...
namespace FsInfoCat.Local.Model
{
    ...
    public class BinaryPropertySet : LocalDbEntity, ILocalBinaryPropertySet, IEquatable<BinaryPropertySet>
    {
        ...
        public virtual M.MD5Hash? Hash { get; set; }
        ...
        IEnumerable<M.IFile> M.IBinaryPropertySet.Files => Files.Cast<M.IFile>();

        IEnumerable<ILocalRedundantSet> ILocalBinaryPropertySet.RedundantSets => RedundantSets.Cast<ILocalRedundantSet>();

        IEnumerable<M.IRedundantSet> M.IBinaryPropertySet.RedundantSets => RedundantSets.Cast<M.IRedundantSet>();
...
#pragma warning restore CS1591 // Missing XML comment for publicly visible type or member
        ...
        internal static void OnBuildEntity(EntityTypeBuilder<BinaryPropertySet> obj) => obj.Property(nameof(Hash)).HasConversion(M.MD5Hash.Converter);
        ...
        internal static async Task<BinaryPropertySet> GetBinaryPropertySetAsync(LocalDbContext dbContext, long length, CancellationToken cancellationToken)
        {
            ...
            if (bps is null)
            {
                ...
                {
                    Length = length,
                    Hash = (length == 0L) ? (M.MD5Hash?)await M.MD5Hash.CreateAsync(System.IO.Stream.Null, cancellationToken) : null
                });
        ...
        internal static async Task<BinaryPropertySet> GetBinaryPropertySetAsync(LocalDbContext dbContext, long length, M.MD5Hash hash, CancellationToken cancellationToken)
        {
        ...
        protected bool ArePropertiesEqual([DisallowNull] ILocalBinaryPropertySet other) => ArePropertiesEqual((M.IBinaryPropertySet)other) &&
        ...
        /// Checks for equality by comparing property values.
        /// </summary>
        /// <param name="other">The other <see cref="M.IBinaryPropertySet" /> to compare to.</param>
        /// <returns><see langword="true" /> if properties are equal; otherwise, <see langword="false" />.</returns>
        protected bool ArePropertiesEqual([DisallowNull] M.IBinaryPropertySet other) => CreatedOn == other.CreatedOn &&
            ModifiedOn == other.ModifiedOn &&
            Length == other.Length &&
            EqualityComparer<M.MD5Hash?>.Default.Equals(Hash, other.Hash);
        ...
        /// Tests whether the current database entity is equal to another.
        /// </summary>
        /// <param name="other">The <see cref="M.IBinaryPropertySet" /> to compare to.</param>
        /// <returns><see langword="true" /> if the <paramref name="other"/> entity is equal to the current entity; otherwise, <see langword="false" />.</returns>
        public bool Equals(M.IBinaryPropertySet other)
        {
            if (other is null) return false;
            if (other is BinaryPropertySet binaryPropertySet) return Equals(binaryPropertySet);
            Guid? id = _id;
...
#pragma warning disable CS1591 // Missing XML comment for publicly visible type or member
        public override bool Equals(object obj)
        {
            ...
            if (obj is M.IBinaryPropertySet propertySet)
            {
                Guid? id = _id;
