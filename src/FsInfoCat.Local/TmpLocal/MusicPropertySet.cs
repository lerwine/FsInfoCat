...
#pragma warning disable CS1591 // Missing XML comment for publicly visible type or member
            ...
            CancellationToken cancellationToken)
        {
            ...
            M.IMusicProperties currentProperties = await fileDetailProvider.GetMusicPropertiesAsync(cancellationToken);
        ...
        public bool Equals(M.IMusicPropertySet other)
        {
            if (other is null) return false;
            if (other is MusicPropertySet propertySet) return Equals(propertySet);
            if (TryGetId(out Guid id)) return other.TryGetId(out Guid id2) && id.Equals(id2);
            return !other.TryGetId(out _) && (other is ILocalMusicPropertySet local) ? ArePropertiesEqual(local) : ArePropertiesEqual(other);
        }

        public override bool Equals(M.IMusicPropertiesRow other)
        {
            if (other is null) return false;
            if (other is MusicPropertySet propertySet) return Equals(propertySet);
            if (TryGetId(out Guid id)) return other.TryGetId(out Guid id2) && id.Equals(id2);
        ...
        public override bool Equals(M.IMusicProperties other)
        {
            if (other is null) return false;
            if (other is MusicPropertySet propertySet) return Equals(propertySet);
            if (other is M.IMusicPropertiesRow row)
            {
        ...
        public override bool Equals(object obj)
        {
            ...
            if (obj is M.IMusicPropertiesRow row)
            {
                if (TryGetId(out Guid id)) return row.TryGetId(out Guid id2) && id.Equals(id2);
                return !row.TryGetId(out _) && (row is ILocalMusicPropertiesRow localRow) ? ArePropertiesEqual(localRow) : ArePropertiesEqual(row);
            }
            return obj is M.IMusicProperties properties && ArePropertiesEqual(properties);
        }
