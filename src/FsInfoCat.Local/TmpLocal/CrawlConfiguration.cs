...
#pragma warning disable CS1591 // Missing XML comment for publicly visible type or member
        ...
        M.ISubdirectory M.ICrawlConfiguration.Root => Root;

        IEnumerable<M.ICrawlJobLog> M.ICrawlConfiguration.Logs => Logs.Cast<M.ICrawlJobLog>();
        ...
        private async Task<M.CrawlStatus?> CrawlAsync<T>(LocalDbContext dbContext, T crawlContext, Subdirectory entity, IActivityProgress progress, Func<T, Subdirectory, (FileInfo[] Files, T[] Directories, M.CrawlStatus? Status)> crawl)
            where T : ICrawlContext
        {
            progress.Token.ThrowIfCancellationRequested();
            (FileInfo[] files, T[] subdirectories, M.CrawlStatus? status) = crawl(crawlContext, entity);
            if (status.HasValue)
                return status.Value;
            ...
            foreach (T ctx in subdirectories)
            {
                ...
                if (subdir is null)
                {
                    subdir = new Subdirectory
                    {
                        ...
                        Parent = entity,
                        Status = M.DirectoryStatus.Incomplete
                    };
        ...
        public static async Task<CrawlConfiguration> CrawlAsync(Guid crawlConfigurationId, IActivityProgress progress)
        {
            ...
            crawlConfiguration.StatusValue = M.CrawlStatus.InProgress;
            //try
            //{
        ...
        /// Checks for equality by comparing property values.
        /// </summary>
        /// <param name="other">The other <see cref="M.ICrawlConfiguration" /> to compare to.</param>
        /// <returns><see langword="true" /> if properties are equal; otherwise, <see langword="false" />.</returns>
        protected virtual bool ArePropertiesEqual([DisallowNull] M.ICrawlConfiguration other)
        {
            if (ArePropertiesEqual((M.ICrawlConfigurationRow)other))
            {
        ...
        public bool Equals(M.ICrawlConfiguration other)
        {
            if (other is null) return false;
            if (other is CrawlConfiguration crawlConfiguration) return Equals(crawlConfiguration);
            if (TryGetId(out Guid id)) return other.TryGetId(out Guid id2) && id.Equals(id2);
        ...
        public override bool Equals(object obj)
        {
            ...
            if (obj is M.ICrawlConfigurationRow row)
            {
                ...
                if (row is M.ICrawlConfiguration crawlConfig)
