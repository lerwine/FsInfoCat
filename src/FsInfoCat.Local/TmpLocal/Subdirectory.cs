...
#pragma warning restore CS0659 // Type overrides Object.Equals(object o) but does not override Object.GetHashCode()
    {
        ...
        /// <remarks>If this is <see langword="null" />, then <see cref="M.ISubdirectory.Parent" /> should not be null, and vice-versa.</remarks>
        ...
        M.IVolume M.ISubdirectory.Volume { get => Volume; }

        IEnumerable<ILocalFile> ILocalSubdirectory.Files => Files.Cast<ILocalFile>();

        IEnumerable<M.IFile> M.ISubdirectory.Files => Files.Cast<M.IFile>();

        IEnumerable<ILocalSubdirectory> ILocalSubdirectory.SubDirectories => SubDirectories.Cast<ILocalSubdirectory>();

        IEnumerable<M.ISubdirectory> M.ISubdirectory.SubDirectories => SubDirectories.Cast<M.ISubdirectory>();

        ILocalCrawlConfiguration ILocalSubdirectory.CrawlConfiguration { get => CrawlConfiguration; }

        M.ICrawlConfiguration M.ISubdirectory.CrawlConfiguration { get => CrawlConfiguration; }
        ...
        IEnumerable<M.ISubdirectoryAccessError> M.ISubdirectory.AccessErrors => AccessErrors.Cast<M.ISubdirectoryAccessError>();
        ...
        M.ISubdirectory M.ISubdirectory.Parent => Parent;
        ...
        IEnumerable<M.IPersonalSubdirectoryTag> M.ISubdirectory.PersonalTags => PersonalTags.Cast<M.IPersonalSubdirectoryTag>();

        IEnumerable<M.IPersonalTag> M.IDbFsItem.PersonalTags => PersonalTags.Cast<M.IPersonalTag>();
        ...
        IEnumerable<M.ISharedSubdirectoryTag> M.ISubdirectory.SharedTags => SharedTags.Cast<M.ISharedSubdirectoryTag>();
...
#pragma warning restore CS1591 // Missing XML comment for publicly visible type or member
        {
            ...
            switch (deletionOption)
            {
                case ItemDeletionOption.Default:
                    ...
                    {
                        if (target.Status == M.DirectoryStatus.Deleted)
                            return false;
                        shouldDelete = false;
            ...
            target.Status = M.DirectoryStatus.Deleted;
            return false;
        }
...
#pragma warning restore CS1591 // Missing XML comment for publicly visible type or member
        {
            ...
            if (directoryInfo.Parent is null)
            {
                ...
                if (parentVolume.State == EntityState.Added)
                {
                    result = new Subdirectory
                    {
                        ...
                        Status = markNewAsCompleted ? M.DirectoryStatus.Complete : M.DirectoryStatus.Incomplete
                    };
            ...
            else
            {
                ...
                else
                {
                    ...
                    DbFile[] files = await (from f in dbContext.Files where f.ParentId == parentId && f.Name == name && f.Status != M.FileCorrelationStatus.Deleted select f).ToArrayAsync(cancellationToken);
                    if (files.Length > 0)
                    {
                        ...
                        if (names.Length == 0 || (files = files.Where(f => !names.Contains(f.Name)).ToArray()).Length > 0)
                        {
                            foreach (DbFile f in files)
                                f.Status = M.FileCorrelationStatus.Deleted;
                            _ = await dbContext.SaveChangesAsync(cancellationToken);
                        }
                    }
                    ...
                    else if (subdirectories.Length > 1)
                    {
                        ...
                        {
                            if ((subdirectories = subdirectories.Where(d => d.Status != M.DirectoryStatus.Deleted && !(ReferenceEquals(d, result) || names.Contains(d.Name))).ToArray()).Length > 0)
                            {
                                foreach (Subdirectory d in subdirectories)
                                    await DeleteAsync(d, dbContext, cancellationToken);
                                await dbContext.SaveChangesAsync(cancellationToken);
                            }
                            if (result.Status == M.DirectoryStatus.Deleted)
                                result.Status = markNewAsCompleted ? M.DirectoryStatus.Complete : M.DirectoryStatus.Incomplete;
...
#pragma warning restore CS1591 // Missing XML comment for publicly visible type or member
            if (dbEntry.Context is not LocalDbContext dbContext)
                throw new InvalidOperationException();
            dbEntry.Entity.Status = M.DirectoryStatus.Incomplete;
...
#pragma warning restore CS1591 // Missing XML comment for publicly visible type or member
            ...
            switch (Status)
            {
                case M.DirectoryStatus.Deleted:
                case M.DirectoryStatus.Incomplete:
                    return;
            }
        ...
        public void SetError(LocalDbContext dbContext, M.ErrorCode errorCode, Exception exception, string message = null)
        {
            ...
            Status = M.DirectoryStatus.AccessError;
        }
...
#pragma warning restore CS1591 // Missing XML comment for publicly visible type or member
        ...
        /// Tests whether the current database entity is equal to another.
        /// </summary>
        /// <param name="other">The <see cref="M.ISubdirectory" /> to compare to.</param>
        /// <returns><see langword="true" /> if the <paramref name="other"/> entity is equal to the current entity; otherwise, <see langword="false" />.</returns>
        public bool Equals(M.ISubdirectory other)
        {
            if (other is null) return false;
            if (other is Subdirectory subdirectory) return Equals(subdirectory);
            if (TryGetId(out Guid id1)) return other.TryGetId(out Guid id2) && id1.Equals(id2);
...
#pragma warning restore CS1591 // Missing XML comment for publicly visible type or member
        {
            ...
            return obj is M.ISubdirectoryRow row && (TryGetId(out Guid id1) ? row.TryGetId(out Guid id2) && id1.Equals(id2) :
