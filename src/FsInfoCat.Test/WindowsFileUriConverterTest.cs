    ...
    public class WindowsFileUriConverterTest
    {
        ...
        public static IEnumerable<TestCaseData> GetToFileSystemPathTestCases() => _testItems.Items.Select(testDataItem => testDataItem.Windows.AbsoluteUrl)
            .Where(u => !(u is null || u.LocalPath is null) && u.IsFileScheme()).Select(u =>
                ...
                .Concat(_testItems.Items.Select(testDataItem => testDataItem.Windows.RelativeUrl).Where(u => !(u is null || u.LocalPath is null)).Select(u =>
                new TestCaseData(u.Owner.Owner.InputString).Returns(TestResultBuilder.CreateTestResult(u.LocalPath.Path).ToTestResultString())));
        ...
        {
            string returnValue = WindowsFileUriConverter.INSTANCE.ToFileSystemPath(fileUriString);
