...
namespace FsInfoCat
{
    public class NullIfWhiteSpaceStringCoersion : ICoersion<string>
    {
        private static string NullIfWhitespace(string source) =>
            (source is not null && source.Length > 0 && source.Any(c => !(char.IsWhiteSpace(c) || char.IsControl(c)))) ? source : null;

        private static string WhitespaceToNull([NotNull] string source) =>
            (source.Length > 0 && source.Any(c => !(char.IsWhiteSpace(c) || char.IsControl(c)))) ? source : null;

        public static readonly NullIfWhiteSpaceStringCoersion Default = new();

        IEqualityComparer<string> _backingComparer;

        Type ICoersion.ValueType => typeof(string);

        public NullIfWhiteSpaceStringCoersion(IEqualityComparer<string> comparer)
        {
            _backingComparer = comparer ?? StringComparer.InvariantCulture;
        }

        private NullIfWhiteSpaceStringCoersion() : this(null) { }

        public virtual string Cast(object obj) => NullIfWhitespace((string)obj);

        public virtual string Coerce(object obj) => (obj is null) ? "" : WhitespaceToNull((obj is string text) ? text : obj.ToString());

        public virtual string Normalize(string obj) => NullIfWhitespace(obj);
        ...
        public virtual bool TryCoerce(object obj, out string result)
        {
            ...
            {
                try { result = WhitespaceToNull(obj.ToString()); }
                catch
                {
                    result = null;
                    return false;
