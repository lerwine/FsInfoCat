...
namespace FsInfoCat
{
    ...
    public abstract class HostingInitializationHandlerAttribute : Attribute
    {
        ...
        protected static IEnumerable<HandlerInfo> GetHandlers(Type arg1Type, Assembly[] assemblies)
        {
            ...
            {
#if DEBUG
                System.Diagnostics.Debug.WriteLine($"Getting static methods marked with {nameof(ConfigurationBuilderHandlerAttribute)} for {assembly.FullName}");
#endif
                foreach (Type targetType in assembly.GetTypes())
                {
                    foreach (MethodInfo method in targetType.GetMethods(BindingFlags.Public | BindingFlags.NonPublic))
                    {
                        ConfigurationBuilderHandlerAttribute attribute = method.GetCustomAttribute<ConfigurationBuilderHandlerAttribute>();
#if DEBUG
