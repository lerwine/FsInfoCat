...
[Serializable]
public readonly partial struct MD5Hash : IEquatable<MD5Hash>, IConvertible
{
    #region Fields
...
#pragma warning restore CS1591 // Missing XML comment for publicly visible type or member
    ...
    /// <summary>
    /// Asynchronously calculates a <see cref="MD5Hash"/> from a <see cref="Stream"/>.
    /// </summary>
    /// <param name="stream">The stream to calculate the hash from.</param>
    /// <param name="cancellationToken">A token that is to be observed in the event that the asynchronous operation needs to be canceled.</param>
    /// <returns>A <see cref="Task{TResult}"/> that returns the calculated <see cref="MD5Hash"/>.</returns>
    public static async Task<MD5Hash> CreateAsync(Stream stream, CancellationToken cancellationToken)
    {
        // DEFERRED: Use alternate hash
#pragma warning disable CA5351 // Broken cryptographic algorithm MD5
        using MD5 md5 = MD5.Create();
#pragma warning restore CA5351 // Missing XML comment for publicly visible type or member
