    ...
    public struct BinaryDenominatedInt64 : IEquatable<BinaryDenominatedInt64>, IComparable<BinaryDenominatedInt64>, IConvertible
    {
        ...
        public static bool TryParse(string s, out BinaryDenominatedInt64 result)
        {
            if (s is not null && (s = s.Trim()).Length > 0)
            {
                if (s.Length > 3)
                {
                    if (long.TryParse(s[0..^2].Trim(), out long numerator))
                        switch (s[^2..].ToUpper())
                        {
                            case TB_SUFFIX:
                                if (numerator <= TB_MAX && numerator >= TB_MIN)
                ...
                else if (long.TryParse(s, out long numerator))
                {
