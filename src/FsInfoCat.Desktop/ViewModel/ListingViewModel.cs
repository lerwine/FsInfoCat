...
namespace FsInfoCat.Desktop.ViewModel
{
    ...
    [Obsolete("Use ListingViewModel<TEntity, TItemViewModel, TOptions, TEditResult>")]
        ...
        where TEditResult : class, IEntityEditResult<TEntity>
    {
        ...
        protected abstract Task<PageFunction<TEditResult>> GetEditPageAsync(TEditArgs args, [DisallowNull] IWindowsStatusListener statusListener);

        [Obsolete("Use GetEditPageAsync(TEditArgs, IWindowsStatusListener)")]
        protected virtual PageFunction<TEditResult> GetEditPage(TEditArgs args) => null;
        ...
        {
            IWindowsAsyncJobFactoryService jobFactory = Services.GetRequiredService<IWindowsAsyncJobFactoryService>();
            jobFactory.StartNew("Loading database record", "Opening database", (TEditArgs)null, GetEditPageAsync).Task.ContinueWith(task => Dispatcher.Invoke(() =>
            {
                if (task.IsCanceled)
                    return;
                if (task.IsFaulted)
                    OnEditTaskFaulted(task.Exception, null);
                else
                {
                    PageFunction<TEditResult> page = task.Result;
                    if (page is null)
