...
{
    public abstract class AndFilter<TEntity> : Filter<TEntity>
        where TEntity : class
    {
        ...
        /// </summary>
        public static readonly DependencyProperty FiltersProperty = DependencyPropertyBuilder<AndFilter<TEntity>, ObservableCollection<Filter<TEntity>>>
        ...
        public override bool IsMatch(TEntity crawlConfiguration)
        {
            ObservableCollection<Filter<TEntity>> filters = Filters;
            return filters is not null && filters.All(f => f is null || f.IsMatch(crawlConfiguration));
        }
    }
}
