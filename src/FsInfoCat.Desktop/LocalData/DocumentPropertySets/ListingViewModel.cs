...
namespace FsInfoCat.Desktop.LocalData.DocumentPropertySets
{
    public class ListingViewModel : ListingViewModel<DocumentPropertiesListItem, ListItemViewModel, bool?>, INavigatedToNotifiable
    {
        ...
        {
            progress.Report("Reading document property sets from database");
            if (options.HasValue)
            {
                if (options.Value)
        ...
        protected override async Task<EntityEntry> DeleteEntityFromDbContextAsync([DisallowNull] DocumentPropertiesListItem entity, [DisallowNull] LocalDbContext dbContext,
            [DisallowNull] IActivityProgress progress)
        {
            DocumentPropertySet target = await dbContext.DocumentPropertySets.FindAsync(new object[] { entity.Id }, progress.Token);
            if (target is null)
                return null;
        ...
        protected async override Task<PageFunction<ItemFunctionResultEventArgs>> GetDetailPageAsync([DisallowNull] ListItemViewModel item, [DisallowNull] IActivityProgress progress)
        {
            ...
            DocumentPropertySet fs = await dbContext.DocumentPropertySets.FirstOrDefaultAsync(f => f.Id == id, progress.Token);
            if (fs is null)
            {
