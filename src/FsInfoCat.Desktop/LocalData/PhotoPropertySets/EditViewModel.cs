        ...
        INavigatedToNotifiable, INavigatingFromNotifiable
    {
        ...
        protected virtual void OnSaveChangesCommand(object parameter)
        {
            if (ApplyChanges() || IsNew)
            {
                ...
                IAsyncJob<PhotoPropertiesListItem> job = jobFactory.StartNew("Saving changes", "Opening database", IsNew, SaveChangesAsync);
        ...
        private static async Task<PhotoPropertiesListItem> SaveChangesAsync(bool isNew, IWindowsStatusListener statusListener)
        {
