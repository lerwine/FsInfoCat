...
{
    public class ListingViewModel : ListingViewModel<PhotoPropertiesListItem, ListItemViewModel, bool?>, INavigatedToNotifiable
    {
        ...
        {
            progress.Report("Reading photo property sets from database");
            if (options.HasValue)
            {
                if (options.Value)
        ...
        protected override async Task<EntityEntry> DeleteEntityFromDbContextAsync([DisallowNull] PhotoPropertiesListItem entity, [DisallowNull] LocalDbContext dbContext,
            [DisallowNull] IActivityProgress progress)
        {
            PhotoPropertySet target = await dbContext.PhotoPropertySets.FindAsync(new object[] { entity.Id }, progress.Token);
            if (target is null)
                return null;
            EntityEntry entry = dbContext.PhotoPropertySets.Remove(target);
        ...
        protected async override Task<PageFunction<ItemFunctionResultEventArgs>> GetDetailPageAsync([DisallowNull] ListItemViewModel item, [DisallowNull] IActivityProgress progress)
        {
            ...
            PhotoPropertySet fs = await dbContext.PhotoPropertySets.FirstOrDefaultAsync(f => f.Id == id, progress.Token);
            if (fs is null)
            {
