...
namespace FsInfoCat.Desktop.LocalData.MusicPropertySets
{
    public class ListingViewModel : ListingViewModel<MusicPropertiesListItem, ListItemViewModel, bool?>, INavigatedToNotifiable
    {
        ...
        protected override IQueryable<MusicPropertiesListItem> GetQueryableListing(bool? options, [DisallowNull] LocalDbContext dbContext, [DisallowNull] IActivityProgress progress)
        {
            progress.Report("Reading music proeprty sets from database");
            if (options.HasValue)
            {
                if (options.Value)
        ...
        {
            MusicPropertySet target = await dbContext.MusicPropertySets.FindAsync(new object[] { entity.Id }, progress.Token);
            if (target is null)
                return null;
        ...
        protected async override Task<PageFunction<ItemFunctionResultEventArgs>> GetDetailPageAsync([DisallowNull] ListItemViewModel item, [DisallowNull] IActivityProgress progress)
        {
            ...
            MusicPropertySet fs = await dbContext.MusicPropertySets.FirstOrDefaultAsync(f => f.Id == id, progress.Token);
            if (fs is null)
            {
