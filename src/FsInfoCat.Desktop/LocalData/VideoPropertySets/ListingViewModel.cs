...
namespace FsInfoCat.Desktop.LocalData.VideoPropertySets
{
    public class ListingViewModel : ListingViewModel<VideoPropertiesListItem, ListItemViewModel, bool?>, INavigatedToNotifiable
    {
        ...
        protected override IQueryable<VideoPropertiesListItem> GetQueryableListing(bool? options, [DisallowNull] LocalDbContext dbContext, [DisallowNull] IActivityProgress progress)
        {
            progress.Report("Reading video property sets from database");
            if (options.HasValue)
            {
                if (options.Value)
        ...
        protected async override Task<PageFunction<ItemFunctionResultEventArgs>> GetDetailPageAsync([DisallowNull] ListItemViewModel item, [DisallowNull] IActivityProgress progress)
        {
            ...
            VideoPropertySet fs = await dbContext.VideoPropertySets.FirstOrDefaultAsync(f => f.Id == id, progress.Token);
            if (fs is null)
            {
        ...
        protected override async Task<EntityEntry> DeleteEntityFromDbContextAsync([DisallowNull] VideoPropertiesListItem entity, [DisallowNull] LocalDbContext dbContext, [DisallowNull] IActivityProgress progress)
        {
            VideoPropertySet target = await dbContext.VideoPropertySets.FindAsync(new object[] { entity.Id }, progress.Token);
            if (target is null)
                return null;
