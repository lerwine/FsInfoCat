...
{
    public class ListingViewModel : ListingViewModel<MediaPropertiesListItem, ListItemViewModel, bool?>, INavigatedToNotifiable
    {
        ...
        private void UpdatePageTitle(bool? options) => PageTitle = options.HasValue ?
                    (options.Value ? FsInfoCat.Properties.Resources.DisplayName_MediaPropertyGroups_HasFiles :
                    FsInfoCat.Properties.Resources.DisplayName_MediaPropertyGroups_NoExistingFiles) :
                    FsInfoCat.Properties.Resources.DisplayName_MediaPropertyGroups_All;
        ...
        protected override IQueryable<MediaPropertiesListItem> GetQueryableListing(bool? options, [DisallowNull] LocalDbContext dbContext, [DisallowNull] IActivityProgress progress)
        {
            progress.Report("Reading media proeprty sets from database");
            if (options.HasValue)
            {
                if (options.Value)
        ...
        protected override async Task<EntityEntry> DeleteEntityFromDbContextAsync([DisallowNull] MediaPropertiesListItem entity, [DisallowNull] LocalDbContext dbContext,
            [DisallowNull] IActivityProgress progress)
        {
            MediaPropertySet target = await dbContext.MediaPropertySets.FindAsync(new object[] { entity.Id }, progress.Token);
            if (target is null)
                return null;
            EntityEntry entry = dbContext.MediaPropertySets.Remove(target);
        ...
        protected async override Task<PageFunction<ItemFunctionResultEventArgs>> GetDetailPageAsync([DisallowNull] ListItemViewModel item, [DisallowNull] IActivityProgress progress)
        {
            ...
            MediaPropertySet fs = await dbContext.MediaPropertySets.FirstOrDefaultAsync(f => f.Id == id, progress.Token);
            if (fs is null)
            {
