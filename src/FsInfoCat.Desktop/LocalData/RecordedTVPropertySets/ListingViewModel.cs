...
{
    public class ListingViewModel : ListingViewModel<RecordedTVPropertiesListItem, ListItemViewModel, bool?, ItemEditResult>, INavigatedToNotifiable
    {
        ...
        protected async override Task<PageFunction<ItemEditResult>> GetDetailPageAsync([DisallowNull] ListItemViewModel item, [DisallowNull] IWindowsStatusListener statusListener)
        {
            ...
            if (item is null)
                viewModel = new(new RecordedTVPropertySet(), null);
            else
            {
                using IServiceScope serviceScope = Services.CreateScope();
                using LocalDbContext dbContext = serviceScope.ServiceProvider.GetRequiredService<LocalDbContext>();
                Guid id = item.Entity.Id;
                RecordedTVPropertySet fs = await dbContext.RecordedTVPropertySets.FirstOrDefaultAsync(f => f.Id == id, statusListener.CancellationToken);
                if (fs is null)
                {
