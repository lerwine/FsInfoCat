...
{
    public class ListingViewModel : ListingViewModel<RecordedTVPropertiesListItem, ListItemViewModel, bool?>, INavigatedToNotifiable
    {
        ...
        private void UpdatePageTitle(bool? options) => PageTitle = options.HasValue ?
                    (options.Value ? FsInfoCat.Properties.Resources.DisplayName_RrecordedTVPropertyGroups_HasFiles :
                    FsInfoCat.Properties.Resources.DisplayName_RrecordedTVPropertyGroups_NoExistingFiles) :
                    FsInfoCat.Properties.Resources.DisplayName_RrecordedTVPropertyGroups_All;
        ...
        protected override IQueryable<RecordedTVPropertiesListItem> GetQueryableListing(bool? options, [DisallowNull] LocalDbContext dbContext, [DisallowNull] IActivityProgress progress)
        {
            progress.Report("Reading recorded TV property sets from database");
            if (options.HasValue)
            {
                if (options.Value)
        ...
        protected override async Task<EntityEntry> DeleteEntityFromDbContextAsync([DisallowNull] RecordedTVPropertiesListItem entity, [DisallowNull] LocalDbContext dbContext,
            [DisallowNull] IActivityProgress progress)
        {
            RecordedTVPropertySet target = await dbContext.RecordedTVPropertySets.FindAsync(new object[] { entity.Id }, progress.Token);
            if (target is null)
                return null;
            EntityEntry entry = dbContext.RecordedTVPropertySets.Remove(target);
        ...
        protected override bool EntityMatchesCurrentFilter([DisallowNull] RecordedTVPropertiesListItem entity) => !_currentOptions.HasValue || (_currentOptions.Value ? entity.ExistingFileCount > 0L : entity.ExistingFileCount == 0L);
        ...
        protected async override Task<PageFunction<ItemFunctionResultEventArgs>> GetDetailPageAsync([DisallowNull] ListItemViewModel item, [DisallowNull] IActivityProgress progress)
        {
            ...
            RecordedTVPropertySet fs = await dbContext.RecordedTVPropertySets.FirstOrDefaultAsync(f => f.Id == id, progress.Token);
            if (fs is null)
            {
