...
namespace FsInfoCat.Desktop.LocalData.SummaryPropertySets
{
    public class ListingViewModel : ListingViewModel<SummaryPropertiesListItem, ListItemViewModel, bool?>, INavigatedToNotifiable
    {
        ...
        protected override IQueryable<SummaryPropertiesListItem> GetQueryableListing(bool? options, [DisallowNull] LocalDbContext dbContext, [DisallowNull] IActivityProgress progress)
        {
            progress.Report("Reading summary property sets from database");
            if (options.HasValue)
            {
                if (options.Value)
        ...
        {
            SummaryPropertySet target = await dbContext.SummaryPropertySets.FindAsync(new object[] { entity.Id }, progress.Token);
            if (target is null)
                return null;
        ...
        protected async override Task<PageFunction<ItemFunctionResultEventArgs>> GetDetailPageAsync([DisallowNull] ListItemViewModel item, [DisallowNull] IActivityProgress progress)
        {
            ...
            SummaryPropertySet fs = await dbContext.SummaryPropertySets.FirstOrDefaultAsync(f => f.Id == id, progress.Token);
            if (fs is null)
            {
