...
namespace FsInfoCat.UnitTests
{
    ...
    public class VolumeIdentifierUnitTest
    {
        ...
        public record ConstructorResultExpected
        {
            ...
            internal static object[] CreateTestData(Guid uuid) => [uuid, new ConstructorResultExpected { UUID = uuid, Location = new Uri($"urn:uuid:{uuid:d}", UriKind.Absolute) }];
            internal static object[] CreateTestData(uint serialNumber) => [serialNumber, new ConstructorResultExpected { SerialNumber = serialNumber, Location = new Uri($"urn:volume:id:{ (serialNumber >> 16):X4}-{(serialNumber & 0x0ffff):X4}", UriKind.Absolute) }];
            internal static object[] CreateTestData(string text, Uri location) => [text, new ConstructorResultExpected { Location = location }];
            internal static object[] CreateTestData(string text, Guid uuid) => [text, new ConstructorResultExpected { UUID = uuid, Location = new Uri($"urn:uuid:{uuid:d}", UriKind.Absolute) }];
            internal static object[] CreateTestData(string text, uint serialNumber) => [text, new ConstructorResultExpected { SerialNumber = serialNumber, Location = new Uri($"urn:volume:id:{(serialNumber >> 16):X4}-{(serialNumber & 0x0ffff):X4}", UriKind.Absolute) }];
            internal static object[] CreateEmptyTestData(string text) => [text, new ConstructorResultExpected { IsEmpty = true, Location = new("", UriKind.Relative) }];
            internal static object[] CreateArgumentOutOfRangeExceptionTestData(string text) => [text, new ConstructorResultExpected { IsArgumentOutOfRangeException = true }];
            internal static object[] CreateTestData(Uri uri, Uri location) => [uri, new ConstructorResultExpected { Location = location }];
            internal static object[] CreateTestData(Uri uri, Guid uuid) => [uri, new ConstructorResultExpected { UUID = uuid, Location = new Uri($"urn:uuid:{uuid:d}", UriKind.Absolute) }];
            internal static object[] CreateTestData(Uri uri, uint serialNumber) => [uri, new ConstructorResultExpected { SerialNumber = serialNumber, Location = new Uri($"urn:volume:id:{(serialNumber >> 16):X4}-{(serialNumber & 0x0ffff):X4}", UriKind.Absolute) }];
            internal static object[] CreateEmptyTestData(Uri uri) => [uri, new ConstructorResultExpected { IsEmpty = true, Location = new("", UriKind.Relative) }];
            internal static object[] CreateArgumentOutOfRangeExceptionTestData(Uri uri) => [uri, new ConstructorResultExpected { IsArgumentOutOfRangeException = true }];
        }
    }
}
