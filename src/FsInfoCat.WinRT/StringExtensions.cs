...
namespace FsInfoCat
{
    public static class StringExtensions
    {
        ...
        public static readonly Regex AbnormalWsRegex = new Regex(@" [\s\p{Z}\p{C}]+|(?! )[\s\p{Z}\p{C}]+", RegexOptions.Compiled);

        public static readonly Regex OuterWsRegex = new Regex(@"^[\s\p{Z}\p{C}]+|[\s\p{Z}\p{C}]+$", RegexOptions.Compiled);

        public static readonly Regex SafeBreakRegex = new Regex(@"^[\s\p{Z}\p{C}]*[^\s\p{Z}\p{C}]+(?=[\s\p{Z}\p{C}])([\s\p{Z}\p{C}]+[^\s\p{Z}\p{C}]+(?=[\s\p{Z}\p{C}]))*|^[\s\p{Z}\p{C}\p{P}\p{M}]*[^\s\p{Z}\p{C}\p{P}\p{M}]+(?=[\p{P}\p{M}])([\p{P}\p{M}]+[^\p{P}\p{M}]+(?=\p{P}))*[\p{P}\p{M}](?=.)|[\s\p{Z}\p{C}\p{P}\p{M}\p{S}]*[^\s\p{Z}\p{C}\p{S}]+(?=\p{S})(\p{S}+\P{S}+(?=\p{S}))*", RegexOptions.Compiled);
        ...
        public static string DefaultIfNullOrEmpty(this string text, string defaultValue) => string.IsNullOrEmpty(text) ? defaultValue : text;

        public static string DefaultIfNullOrWhiteSpace(this string text, string defaultValue) => string.IsNullOrWhiteSpace(text) ? defaultValue : text;

        public static string GetDefaultIfNullOrEmpty(this string text, Func<string> getDefaultValue) => string.IsNullOrEmpty(text) ? getDefaultValue() : text;

        public static string GetDefaultIfNullOrWhiteSpace(this string text, Func<string> getDefaultValue) => string.IsNullOrWhiteSpace(text) ? getDefaultValue() : text;
        ...
        public static IEnumerable<string> NonEmptyTrimmedElements(this IEnumerable<string> text) => text?.Where(s => !(s is null)).Select(s => s.Trim())
            .Where(s => s.Length > 0);
